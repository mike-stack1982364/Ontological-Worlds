...
(function exposeModeOneConflictMatrix(root, factory) {
  ...
  function installStyles(documentObject){
    ...
    style.textContent=`#conflict-matrix{width:100vw;max-width:none;margin:12px 0 16px;position:relative;left:50%;right:50%;margin-left:-50vw;margin-right:-50vw;padding:14px clamp(14px,2.4vw,34px);box-sizing:border-box;border-top:1px solid #b8c9da;border-bottom:1px solid #b8c9da;background:rgba(255,255,255,.94)}#conflict-matrix[hidden]{display:none!important}.conflict-heading{font-weight:900;letter-spacing:.055em;text-transform:uppercase;margin:0 auto 12px;max-width:1600px;color:#173f67;text-align:center}.conflict-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:clamp(8px,1vw,16px);max-width:1800px;margin:0 auto}.conflict-row{display:grid;grid-template-columns:1fr 1fr;gap:8px;align-items:stretch;margin:0;min-width:0}.conflict-label{grid-column:1/-1;min-height:2.9em;display:flex;align-items:flex-end;justify-content:center;text-align:center;font-size:clamp(.7rem,.85vw,.86rem);font-weight:800;line-height:1.2;color:#30465d;padding:0 4px}.conflict-choice{width:100%;min-width:0;min-height:64px;border-radius:12px;font-size:clamp(.72rem,1vw,.95rem);font-weight:900;line-height:1.08;padding:8px 5px;white-space:normal}.conflict-choice.selected{outline:3px solid #183f67;outline-offset:2px}.conflict-choice[disabled]{opacity:.58}#conflict-submit{display:block;width:min(100%,760px);min-height:50px;margin:14px auto 0;border-radius:11px;font-weight:900;text-transform:uppercase}#conflict-progress,#conflict-score{font-size:.78rem;margin-top:8px;color:#53697e;text-align:center}.response-buttons.conflict-replaced{display:none!important}@media(max-width:1050px){.conflict-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.conflict-row:last-child{grid-column:1/-1;max-width:520px;width:100%;justify-self:center}}@media(max-width:620px){#conflict-matrix{padding-left:10px;padding-right:10px}.conflict-grid{grid-template-columns:1fr;gap:10px}.conflict-row:last-child{grid-column:auto;max-width:none}.conflict-label{min-height:auto;align-items:center}.conflict-choice{min-height:52px}}`;
  ...
  function installMatrixUI(rootObject,app){
    ...
    matrix.innerHTML=`<div class="conflict-heading">Relational conflict matrix</div><div class="conflict-grid">${labels.map((label,index)=>`<div class="conflict-row" data-decision="${index}"><div class="conflict-label">${label}</div><button class="conflict-choice" data-value="1" type="button">${index===3?'Entailed':'Match'}</button><button class="conflict-choice" data-value="0" type="button">${index===3?'Not entailed':'No match'}</button></div>`).join('')}</div><button id="conflict-submit" type="button" disabled>Submit all five decisions</button><div id="conflict-progress" aria-live="polite">0 of 5 decisions entered</div><div id="conflict-score" aria-live="polite"></div>`;
  ...
  return Object.freeze({version:23,LEVELS,ALL_MASKS,analyseAlignment,evaluateConflictMatrix,generateConflictTrial,generateWarmupTrial,evaluateHistory,runAudit,installBrowser});
});