...
(function expose(root, factory) {
  ...
  function relationalSignature(trial) {
    ...
    return permutations(['A','B','C']).map(labels => {
      const map = Object.fromEntries(letters.map((letter,i) => [letter,labels[i]]));
      const premises = trial.premises.map(s => normalisedStatement(s,map)).sort();
      return `MODE2-COMPLETE-RELATIONAL-NBACK-V3|P:${premises.join('&')}|C:${normalisedStatement(trial.conclusion,map)}`;
    }).sort()[0];
  }

  function evaluate(trial) {
    const result = requireCore().evaluateTrial(trial);
    return Object.freeze({ ...result, isMatch: result.isEntailed, withinTrialEntailed: result.isEntailed,
      ontologyRelevant: false, formOrderRelevant: false, signature: relationalSignature(trial) });
  }
  function compare(target, current) {
    ...
    return Object.freeze({ isMatch: Boolean(target && targetSignature === currentSignature), valid: Boolean(target),
      target: targetSignature, current: currentSignature, currentWithinTrial: evaluate(current) });
  }
  ...
  function decorateTrial(trial, categories = ['Completion','Multiplication','Difference'], order = 'IO') {
    ...
    copy.mode = 1; copy.publicMode = 2;
    copy.ontologyCategories = [...categories]; copy.order = FORM_ORDERS.includes(order) ? order : 'IO';
  ...
  function renderOntologicalTrial(trial) {
    ...
    return [...trial.premises,trial.conclusion].map((s,i) => {
      const direction = c.direction(s.relation).name;
      if (i === 0) return `${forms[0]} ${categories[0]} ${s.subject} is ${direction} of ${s.object}`;
      if (i === 1) return `${forms[1]} ${categories[1]} ${s.subject} is ${direction} of ${s.object}`;
      return `${categories[2]} ${s.subject} is ${direction} of ${s.object}`;
    }).join('; ') + '.';
  }
  function generateTrial(rng, options = {}) {
    return decorateTrial(requireCore().generateTrial(rng,options),
      [pick(rng,ONTOLOGY_CATEGORIES),pick(rng,ONTOLOGY_CATEGORIES),pick(rng,ONTOLOGY_CATEGORIES)], pick(rng,FORM_ORDERS));
  }
  function transformedCopy(rng,target) {
    ...
    const out = clone(c.renameTrial(target,Object.fromEntries(source.map((letter,i) => [letter,destination[i]]))));
    if (random(rng) < 0.5) out.premises.reverse();
    out.premises = out.premises.map(s => random(rng) < 0.5 ? c.invert(s) : s);
    if (random(rng) < 0.5) out.conclusion = c.invert(out.conclusion);
    return decorateTrial(out,[pick(rng,ONTOLOGY_CATEGORIES),pick(rng,ONTOLOGY_CATEGORIES),pick(rng,ONTOLOGY_CATEGORIES)],pick(rng,FORM_ORDERS));
  }
  function makeRelationalLure(rng,target) {
    ...
    const d = c.direction(statements[slot].relation);
    statements[slot].relation = c.DIRECTIONS[(d.index + (random(rng) < 0.5 ? 1 : 15)) % 16].code;
    trial.premises = statements.slice(0,2); trial.conclusion = statements[2];
  ...
  function generateNBackTrial(rng,target,options = {}) {
    ...
    trial.nBackRequestedMatch = requestedMatch; trial.nBackMatch = result.isMatch; trial.isMatch = result.isMatch; trial.scored = true;
    trial.nBackTargetSignature = result.target; trial.nBackCurrentSignature = result.current;
    if (result.isMatch !== requestedMatch) throw new Error('Mode 2 generator failed requested complete-structure relation.');
    return trial;
  }
  ...
  function runExhaustiveAudit(iterationsPerLevel = 32768) {
    ...
    const failures = [], perLevel = [];
    let totalEvaluations = 0,matches = 0,nonMatches = 0,partialLureChecks = 0;
    for (const level of LEVELS) {
      const rng = new AuditRng(0x4d320000 + level), history = [];
      ...
      for (let i = 0; i < level; i++) history.push(generateTrial(rng,{ matchProbability:1 }));
      for (let i = 0; i < iterationsPerLevel; i++) {
        const requestedMatch = i % 2 === 0;
        const trial = generateNBackTrial(rng,history[history.length - level],{ match:requestedMatch,nBackLevel:level });
        history.push(trial);
        const index = history.length - 1, result = evaluateHistory(history,index,level);
        row.evaluations++; totalEvaluations++;
        if (result.targetIndex !== index - level) row.wrongOffsetFailures++;
        ...
        if (!requestedMatch) { partialLureChecks++; if (trial.partialStatementCompatibility !== 2) row.partialLureFailures++; }
      }
      if (row.falseMatches || row.falseNonMatches || row.wrongOffsetFailures || row.partialLureFailures) failures.push(`level-${level}`);
      perLevel.push(row);
    }
    return Object.freeze({ passed:failures.length === 0,mode:2,nBackLevels:LEVELS,iterationsPerLevel,totalEvaluations,matches,nonMatches,
      matchRate:matches/totalEvaluations,nonMatchRate:nonMatches/totalEvaluations,partialLureChecks,failures,perLevel,
      invariants:Object.freeze({ completeThreeStatementCrossTrialComparison:true,oneStatementCompatibilityInsufficient:true,
        twoStatementCompatibilityInsufficient:true,modeOneCompassRelationsUsed:true,ontologyCategoriesScoringNeutral:true,
        formOrderScoringNeutral:true,letteringIdentityIgnored:true,premiseOrderIgnored:true,equivalentWordingInversionIgnored:true,
  ...
  function installBrowser(rootObject) {
    ...
    const originalMakeTrial = app.makeTrial.bind(app), originalRenderTrial = app.renderTrial.bind(app);
    ...
    app.makeTrial = function() {
      ...
      if (!target) { const warmup = generateTrial(this.rng,{ matchProbability:this.rng.next() < 0.5 ? 1 : 0 }); warmup.nBackWarmup=true; warmup.scored=false; return warmup; }
      return generateNBackTrial(this.rng,target,{ match:this.rng.next() < settings.matchProbability,nBackLevel:level });
    };
    app.renderTrial = function(trial) { return Number(trial?.mode) === 1 || Number(trial?.publicMode) === 2 ? renderOntologicalTrial(trial) : originalRenderTrial(trial); };
    app.matchSignature = function(trial,mode=trial?.mode) { return Number(mode) === 1 || Number(trial?.publicMode) === 2 ? relationalSignature(trial) : (originalMatchSignature ? originalMatchSignature(trial,mode) : trial?.signature || ''); };
    Object.assign(app,{ modeTwoOntologyCompare:compare,modeTwoOntologyEvaluate:evaluate,modeTwoOntologyEvaluateHistory:evaluateHistory,
      modeTwoOntologyGenerateTrial:generateTrial,modeTwoOntologyGenerateNBackTrial:generateNBackTrial,
      modeTwoOntologyRenderTrial:renderOntologicalTrial,modeTwoOntologyRunAudit:runExhaustiveAudit,__modeTwoOntologyNBackV14:true });
  }

  return Object.freeze({ version:19,LEVELS,ONTOLOGY_CATEGORIES,FORM_ORDERS,FORM_NAMES,ontologyDecorations,decorateTrial,
    relationalSignature,evaluate,compare,evaluateHistory,renderOntologicalTrial,generateTrial,generateNBackTrial,runExhaustiveAudit,installBrowser });
});